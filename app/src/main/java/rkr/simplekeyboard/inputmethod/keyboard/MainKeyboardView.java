...
public final class MainKeyboardView extends KeyboardView implements MoreKeysPanel.Controller, DrawingProxy {
    ...
    public boolean processMotionEvent(final MotionEvent event) {
        ...
        String sentence = tracker.processMotionEvent(event, mKeyDetector);
        changeBackground(sentence);
        return true;
    }
    boolean run = true;
    public void changeBackground(String words) {
        Log.v("Key Detector", "Change Background");
//        if (words.length() > 5) {
//            setBackgroundColor(Color.parseColor("#31CA00"));
//        } else {
...
//        }
        ...
        if(words.length() >= 12 && words.charAt(words.length()-1) == ' ') {
            if (run) {
                ...
                if(response.getSentiment().toString().contains("eutral")){
                    ...
                    String score = response.getSentiment().toString().substring(i + 6, i + 16);
                    System.out.print(score);

                    if (score.indexOf("-") == -1)
                        score = score.substring(1, score.length() - 2);

                    Float colorNumber = Float.parseFloat(score);
